...
def train_main():
    ...
    train_loader = load_data("/raid/home/dvl/mpresutto/vol/DynMM/FusionDynMM/datasets/xdviolence/train")
    valid_loader = load_data("/raid/home/dvl/mpresutto/vol/DynMM/FusionDynMM/datasets/xdviolence/train")
    ...
    list_loss = []
    for epoch in range(0,200):#range(int(start_epoch), args.epochs):
...
def train_one_epoch(model, train_loader, device, optimizer, loss_function_train,
    ...
    for i, (frame,spects,label) in tqdm(enumerate(train_loader)):
        ...
        if modality in ['rgbd', 'rgb']:
            ...
            for spec in spects:
                spect_ = torch.from_numpy(np.asarray(spec))
                spect_ = spect_[:, None,:]
        ...
        # forward pass
        if modality == 'rgbd':
            spect = torch.index_select(spect, 1, torch.tensor([0]).to(device))
            #print("\nimage len\n",len(image))
            #print("\spect len\n",len(spect))
            ...
            spects = spects.float()
            spects = spects.cuda()
            pred_scales = model(images, spects[i])
        ...
        losses = loss_function_train_(pred_scales, label_vec)
        print("\nloss: ",losses)
...
def validate(model, valid_loader, device, confusion_matrices,
    ...
    total_count = 0
    for i, (frames,spects,labels) in tqdm(enumerate(valid_loader)):
        ...
        with torch.no_grad():
            ...
            for j in range(len(labels[i])):
                print(prediction_res[j],labels[i][j])
                if prediction_res[j] == labels[i][j]:
                    count += 1
            print(count)
            acc = count/len(labels[i])
            print("batch_accuracy: ", count/len(labels[i]))
